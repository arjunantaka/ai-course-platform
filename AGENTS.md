...
| `bun run build` | Production build (adapter-node, output `build/`) |
| `bun run start` | Run the production build (`HOST`/`ORIGIN` from env) |
| `bun run check` | svelte-check; must stay 0 errors / 0 warnings |
| `bun run db:generate` | Generate Drizzle migrations from the schema |

## Overview

KrasCourse is a no-account learning platform. An admin creates a course from one
topic; an AI pipeline (outline, then lessons, then quizzes) generates the content
into SQLite through Drizzle ORM. Public pages: home, catalog with search, course
detail, lesson reader, and a quiz per module. Learner progress (finished lessons,
quiz scores) lives in `localStorage` via `src/lib/progress.svelte.ts`; it is never
sent to the server.

- Stack: SvelteKit 2, Svelte 5 runes, Tailwind CSS v4, Drizzle ORM + SQLite (`bun:sqlite`), Bun runtime.
- `src/hooks.server.ts` runs migrations on boot and recovers courses stuck in a generating state after a restart. It also guards `/admin` and `/api/admin/*` with the `ADMIN_TOKEN` cookie.
- Public lesson content is rendered server-side with `marked` + `sanitize-html` (`src/lib/server/markdown.ts`). The admin preview renders client-side without sanitizing (trusted input).
- Design direction lives in `DESIGN.md`; the anti-slop audit is in `anti-slop/`. Follow them for any UI or copy change.
...
- Never bind the server to `0.0.0.0`; keep `HOST=127.0.0.1` behind a reverse proxy.
- Keep changes focused: one purpose per change; separate features, refactors, and fixes.

## Coding Habits

1. **Keep the main path easy to follow.** Use guard clauses and early returns; never bury the happy path in nested conditions.
2. **Name things by meaning.** `pendingOrder`, not `data`; function names state their purpose.
3. **Keep external systems behind a boundary.** Wrap third-party APIs in an adapter and map their fields into internal models in one place.
4. **Make invalid states harder to represent.** Model valid states in types instead of optional-everything fields.
5. **Separate decisions from actions.** Pure functions for rules (eligibility, pricing, validation); side effects run after the decision.
6. **Make errors useful.** Machine-readable code plus a human-readable message and diagnostic context; never log secrets.
7. **Keep your changes focused.** Atomic changes with a single purpose; easier to review, test, and roll back.
...
Objects and arrays (`$state({...})` or `$state([...])`) are made deeply reactive, meaning mutation will trigger updates. This has a trade-off: in exchange for fine-grained reactivity, the objects must be proxied, which has performance overhead. For large objects that are only ever reassigned (rather than mutated), use `$state.raw` instead. This is often the case with API responses, for example.
...
`$inspect.trace` is a debugging tool for reactivity. If something is not updating properly or running more than it should, add `$inspect.trace(label)` as the first line of an `$effect` or `$derived.by` (or any function they call) to trace their dependencies and discover which one triggered the update.
...
If you need to attach listeners to `window` or `document`, use `<svelte:window>` and `<svelte:document>`:
...
Prefer [keyed each blocks](each#Keyed-each-blocks); this improves performance by allowing Svelte to surgically insert or remove items rather than updating the DOM belonging to existing items.
...
If you have a JS variable that you want to use inside CSS, set a CSS custom property with the `style:` directive.
